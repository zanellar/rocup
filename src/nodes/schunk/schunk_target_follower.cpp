...
{
    ros::init(argc, argv, "schunk_target_follower");
     ...
     */
    robot_name = nh->param<std::string>("robot_name", "schunk_pg70");
     ...
     * Robot Model
     */
    std::string base_link = robot_name + "/schunk_pg70_base";
    std::string ee_link = robot_name + "/schunk_pg70_finger1";
    ...
    while (nh->ok())
    {
        ...
        else if (followMachine.state == FollowMachineState::FOLLOWING)
        {
            ...
            {
                if (followMachine.checkMovement(followMachine.target_joint))
                {
        ...
        tf_broadcaster->sendTransform(tf::StampedTransform(tool_tf, ros::Time::now(), ee_link, robot_name + "/tool"));
